...
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $achats=$bdd->prepare('SELECT id_fournisseur,
   id_entreprise,
   id,
   marchandise,
   quantite,
   ...
   montant_emballage,
   montant_avance,
   monyen_payement,montant_payement,  DATE_FORMAT(dates,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM achats WHERE id=? ORDER BY id DESC ');
$achats->execute(array($getid));
   ?>
