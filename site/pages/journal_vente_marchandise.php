...
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $vnt_marchandise=$bdd->prepare('SELECT  id_client,
   id_entreprise,
   marchandise,
   quantite,
   id,
   ...
   DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM ventes WHERE id_entreprise=? ORDER BY id DESC ');
   $vnt_marchandise->execute(array($_SESSION['id']));
   ?>
         ...
         ?>
          <div class="container ">
            <div class="row mb-4">
                ...
                <a style='color:black;   text-decoration: none;' href="visteurs.php?page=journal_chaque_marchandise_vendu&id=<?=$donnees['id'] ?>">
