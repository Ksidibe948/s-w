...
if (isset($_GET['id'])) {
   ...
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare('SELECT * FROM   autres_charges WHERE id=? ORDER BY id DESC');
...
<?php
       ...
       {   
            ...
            $req=$bdd->prepare('UPDATE   autres_charges SET nom_operation=? WHERE id=?');
            $req->execute(array($nom_operation,$getid));
            header('Location:visteurs.php?page=autres_charges&id='.$getid);
            $req=$bdd->prepare('UPDATE   autres_charges SET montant_th=? WHERE id=?');
            $req->execute(array($montant_th,$getid));
            header('Location:visteurs.php?page=autres_charges&id='.$getid);
            $req=$bdd->prepare('UPDATE   autres_charges SET taux_tva=? WHERE id=?');
            $req->execute(array($taux_tva,$getid));
            header('Location:visteurs.php?page=autres_charges&id='.$getid);
            $req=$bdd->prepare('UPDATE   autres_charges SET numero_banque=? WHERE id=?');
            $req->execute(array($numero_banque,$getid));
            header('Location:visteurs.php?page=autres_charges&id='.$getid);
            $req=$bdd->prepare('UPDATE   autres_charges SET montant_payer=? WHERE id=?');
