...
if (isset($_GET['id'])) {
   ...
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare('SELECT * FROM   autre_produits WHERE id=? ORDER BY id DESC');
...
<?php
       ...
       {   
            ...
            $banques = $banque['montant'] + $montant_payer; 
            $req=$bdd->prepare('UPDATE banque SET montant=? where id=?');
            $req->execute(array($banques,$numero_banque));


            $req=$bdd->prepare('UPDATE   autre_produits SET nom_operation=? WHERE id=?');
            $req->execute(array($nom_operation,$getid));
            header('Location:visteurs.php?page=autre_produits&id='.$getid);
            $req=$bdd->prepare('UPDATE   autre_produits SET montant_th=? WHERE id=?');
            $req->execute(array($montant_th,$getid));
            header('Location:visteurs.php?page=autre_produits&id='.$getid);
            $req=$bdd->prepare('UPDATE   autre_produits SET taux_tva=? WHERE id=?');
            $req->execute(array($taux_tva,$getid));
            header('Location:visteurs.php?page=autre_produits&id='.$getid);
            $req=$bdd->prepare('UPDATE   autre_produits SET numero_banque=? WHERE id=?');
            $req->execute(array($numero_banque,$getid));
            header('Location:visteurs.php?page=autres_produits&id='.$getid);
            $req=$bdd->prepare('UPDATE   autre_produits SET montant_payer=? WHERE id=?');
        ...
        <div class="collapse navbar-collapse" id="navbar">
          <ul class="navbar-nav mr-auto">
            <li class="nav-item  mr-3">
              <a class="nav-link" href="visteurs.php?page=produits">Produits<span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item active  mr-3">
              <a class="nav-link" href="">Modifier Produit<span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item mr-3">
     ...
     <div class="container">
         <div class="row">
                            ...
                            <div class="card-body">
                              ...
                              <div class="col-lg-3">
                              <input type="text" Value='<?= $donnees ['nom_operation'] ?>' name='nom_operation'class="form-control" id="inputCity "  placeholder="  Ex: gain de change ">
                              </div>
                            </div><hr>
                            <div class="form-group row">
                              <div class="col-lg-2"></div>
                              ...
                              <div class="col-lg-3">
                              <input type="number" Value='<?= $donnees ['montant_th'] ?>'  name='montant_th'class="form-control" id="inputCity "  placeholder="  Ex: 1000 ">
                              </div>
                            </div><hr>
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-lg-3">
                              <input type="number" Value='<?= $donnees ['numero_banque'] ?>'  name='numero_banque'class="form-control" id="inputCity "  placeholder="  Ex: 2 ">
                              </div>
                            </div><hr>
                            <div class="form-group row">
                              <div class="col-lg-2"></div>
